  ...
  useEffect(() => {
    async function loadReceipt() {
      ...
      try {
        ...
        if (res.status === 403) {
          setError(isHi ? "अनधिकृत पहुंच: यह दान रसीद केवल दानदाता या व्यवस्थापक द्वारा देखी जा सकती है।" : "Unauthorized access to this donation receipt.");
          return;
        }
        ...
        setError("नेटवर्क त्रुटि उत्पन्न हुई। कृपया पुनः प्रयास करें।");
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="py-24 text-center text-stone-500 space-y-3">
        ...
        <p className="text-xs font-serif font-bold text-maroon-deep">
          {isHi ? "दान रसीद तैयार हो रही है..." : "Generating official donation receipt..."}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-md mx-auto py-16 px-4 text-center space-y-4">
        ...
        >
          ...
          <span>{isHi ? "दान पृष्ठ पर वापस जाएं" : "Back to Donations"}</span>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="py-8 sm:py-12 px-4 max-w-3xl mx-auto space-y-6">
      ...
      <div className="print:hidden flex items-center justify-between gap-4">
        ...
        >
          <ArrowLeft className="w-3.5 h-3.5" />
          <span>{isHi ? "मुख्य दान पृष्ठ" : "Back to Donations"}</span>
        </Link>
        ...
        >
          ...
          <span>{isHi ? "रसीद प्रिंट / डाउनलोड करें (PDF)" : "Print / Download PDF"}</span>
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl border-2 border-stone-300 shadow-xl overflow-hidden p-8 sm:p-12 space-y-8 text-stone-900 print:border-none print:shadow-none print:p-0 print:m-0">
          ...
          <div className="text-center sm:text-right shrink-0">
            ...
            <div className="mt-2 text-xs text-stone-500 space-y-0.5 font-mono">
              ...
              <div>
                ...
                <span className="text-stone-900">{new Date(receipt.issuedAt).toLocaleDateString(isHi ? "hi-IN" : "en-US", { year: "numeric", month: "short", day: "numeric" })}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-cream-warm/50 p-6 rounded-2xl border border-sandstone-200 text-xs space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 sm:text-right">
              ...
              <p className="text-stone-800 font-serif font-bold text-sm">{isHi ? receipt.causeTitleHi : receipt.causeTitleEn}</p>
        ...
        <div className="bg-cream-warm/30 p-5 rounded-2xl border border-sandstone-200 space-y-2 text-xs text-stone-600 leading-relaxed">
          <div className="flex items-center gap-2 text-maroon-deep font-serif font-bold text-xs">
            <Heart className="w-4 h-4 text-gold-royal shrink-0" />
            <span>{isHi ? "आधिकारिक उद्घोषणा एवं कृतज्ञता" : "Official Acknowledgement"}</span>
          </div>
          <p>{isHi ? receipt.legalNoteHi : receipt.legalNoteEn}</p>
        ...
        <div className="pt-6 border-t border-stone-200 grid grid-cols-1 sm:grid-cols-2 gap-6 items-end text-xs text-stone-500">
          <div className="space-y-1 text-[11px]">
            <p className="font-serif font-bold text-stone-800">॥ श्री ॐ जोरावर परमात्मने नमः ॥</p>
            <p>संपर्क: info@jorawardham.in • दूरभाष: +91-9530106218</p>
